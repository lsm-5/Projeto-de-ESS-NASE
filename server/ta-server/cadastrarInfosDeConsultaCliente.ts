...
export class CadastroDeInfosDeCosultas {
  ...
  criar(info: InfosDeConsultaCliente): InfosDeConsultaCliente {
    var result = null;
    if (this.infoCosultaNaoCadastrada(info.nomeProfissional, info.dataDaConsulta, info.horarioConsulta)) {
  ...
  remover(info:InfosDeConsultaCliente): boolean {
      this.infos = this.infos.filter(a => (a.nomeProfissional != info.nomeProfissional) && (a.dataDaConsulta != info.dataDaConsulta) && (a.horarioConsulta != info.horarioConsulta));
      return true;
  }

  infoCosultaNaoCadastrada(nomeProfissional: string, dataDaConsulta: string, horarioConsulta: string): boolean {
    if(this.infos.find(a => (a.nomeProfissional == nomeProfissional) && (a.dataDaConsulta == dataDaConsulta) && (a.horarioConsulta == horarioConsulta)){
      return false;
    }else{
      return true;
    }
  ...
  atualizar(info: InfosDeConsultaCliente): InfosDeConsultaCliente {
    if (!this.infoCosultaNaoCadastrada(info.nomeProfissional, info.dataDaConsulta, info.horarioConsulta)) {
      var result: InfosDeConsultaCliente = this.infos.find(a => (a.nomeProfissional == info.nomeProfissional) && (a.dataDaConsulta == info.dataDaConsulta) && (a.horarioConsulta == info.horarioConsulta));
